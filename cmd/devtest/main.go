...
func main() {
	...
	// 1) cluster.scan_topology - list all clusters
	{
		tool := tools.ClusterScanTopology()
		res, err := tool.Handler(ctx, nil, &mcp.CallToolParamsFor[tools.ClusterScanTopologyParams]{Arguments: tools.ClusterScanTopologyParams{ListAll: true}})
		if err != nil {
			fmt.Fprintf(os.Stderr, "cluster.scan_topology error: %v\n", err)
			os.Exit(1)
		}
		fmt.Println("=== cluster.scan_topology ===")
